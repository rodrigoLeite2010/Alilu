 ...
 * conhecer detalhes do Axios). Este módulo existe só para os dois lados
 * conversarem sem um importar o outro (evita ciclo de import):
 *
 * - `AuthProvider` registra, uma vez, como renovar o token
 *   (`registerRefreshHandler`) e como encerrar a sessão
 *   (`registerLogoutHandler`).
 * - Os interceptors do Axios usam `getAccessToken` para anexar o header
 *   `Authorization`, e chamam `refreshAccessToken`/`triggerLogout` quando
 *   uma resposta 401 chega.
 */
